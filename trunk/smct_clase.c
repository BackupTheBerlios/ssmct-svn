...
int sop_hasta[500],sop_desde[500];
 ...
 strcpy(name_file,"/tmp/FILE1");
  ...
  fp1=popen("greq -t'Hola, primero conectamos' -e'Dir. IP de tu servidor' -d ssmct.berlios.de -ePuerto -d 80 -c 'DFSI' -c 'IMSI'","r");
  if(!fp1) {
  ...
  sscanf(sp,"%s\v%s\v%s\v%s",servidor,puerto,dfsi,imsi);
  // Averiguar host
 ...
 if(hp==NULL) {
  ...
  if((*dfsi=='F') && (*imsi=='F')) {
     puts(" *** Debes seleccionar 'dfsi' ó 'imsi' ***");
     exit(1);
  }
  if(*dfsi=='T' && *imsi=='T') {
     puts(" *** En cada sesión admite un módulo -dfsi ó imsi- *** ");
     exit(1);
  }
  if(*dfsi=='T') {
    //es dfsi
  ...
  sprintf(peticion,"GET http://%s/%s/hay HTTP/1.0\n\n",servidor,dir);
  ...
  sprintf(peticion,"GET http://%s/%s/%s HTTP/1.0\n\n",servidor,dir,file);
  write(fp,peticion,strlen(peticion));
  *respuesta='\0';
  while(1) {
   leidos=read(fp,res,64000);
