...
#include<sys/time.h>
void fun_enviar(char *,struct sockaddr_in);
int comunica(void);
...
main(int argc, char **argv) {
  ...
  int una_opcion,tipo_note,cordinado,tipo_stop;
  int ficha,ficha_entry,passwd;
  char nombre[250],servidor[250],puerto[30], correg[250],pass[250],temas[250];
  char tipo[500],file[500],asignatura[60];
  struct timeval value;
  long msglen,n;
  int ret,leidos;
  char peticion[500],respuesta[64000],res[64000],name_file[50],cod_file[30];
  char buf[70000],s9[60],buf1[250],dfsi[30],imsi[30],dir[500];
 ...
 char env_resp[60000],sp[500],cod_bscw[50];
...
int sop_hasta[500],sop_desde[500];
 ...
 strcpy(name_file,"/tmp/FILE1");
  ...
  tipo_test=primario=tipo_stop=0;
  strcpy(puerto,"80");
  fp1=popen("greq -t'Hola, primero conectamos' -e'^Dir. IP de tu servidor' -d informatica.iespoligonosur.org -eAsignatura -d dfsi -e'Código BSCW' -d d","r");
  if(!fp1) {
  ...
  sscanf(sp,"%s\v%s\v%s",servidor,asignatura,cod_bscw);
  // Averiguar host
 ...
 //fflush(stdout);
  ...
  sprintf(peticion,"GET http://%s/pub/bscw.cgi/%s/code.txt HTTP/1.0\n\n",servidor,cod_bscw);
  ...
  switch(p[3]) {
    ...
    case '4': saq=1;
              ...
              break;
    case '5': una_opcion=1;
              strcpy(file,"unaOpcion");
              break;
    case '6': tipo_note=1;
              strcpy(file,"notebook");
              break;
    case '9': tipo_stop=1;
              strcpy(file,"stop");
              break;
  ...
  sprintf(peticion,"GET http://%s/pub/bscw.cgi/%s/%s.txt HTTP/1.0\n\n",servidor,p+5,file);
  write(fp,peticion,strlen(peticion));
  *respuesta='\0';
  while(1) {
   leidos=read(fp,res,64000);
 ...
 if(una_opcion) {
    ...
    sprintf(env_resp,"%s:%s:%s",asignatura,file,nombre);
    id=0;
    ejecuta_funciones(env_resp,id,fp,value,val_size,saddr,faddr,
                  faddrlen,cordinado);
   close (fp);
   saddr.sin_port=htons(14500);
   grabar_temporal(env_resp);
   fun_enviar(env_resp,saddr);
   exit(1); 
 }
 if(tipo_note) {
 ...
 if(tipo_stop) {
   printf(" *** No hay Trabajos para resolver ***\n");
   fflush(stdout);
   exit(1);
   sprintf(env_resp,"%s:%s:%s\v",asignatura,file,nombre);
   close (fp);
   saddr.sin_port=htons(14500);
   fp=socket(AF_INET,SOCK_DGRAM,0);
   if(fp==-1) {
     perror("En SOCKET DGRAM");
     exit(1);
   }
   n=sendto(fp,env_resp,strlen(env_resp)+1,0,(struct sockaddr *)&saddr,sizeof(saddr));
   if(n==-1) {
     perror("En sendto DGRAM");
     exit(1);
   }
   close(fp);
 }
 ...
 if(tipo_test) {
    sprintf(env_resp,"%s:%s:%s\n",asignatura,file,nombre);
    id=0;
...
//
void  fun_enviar (char *env_resp,struct sockaddr_in saddr) 
{
   int fp,n;
   saddr.sin_port=htons(14500);
   fp=socket(AF_INET,SOCK_DGRAM,0);
